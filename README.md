...
### Diagram with closed and unlocked as the same thing:

 ``` mermaid
  stateDiagram-v2
  direction LR
  state: "Door"
  open --> closed_unlocked : Closing
  closed_unlocked --> open : Opening
  closed_unlocked --> locked : Locking
  locked --> closed_unlocked : Unlocking
```
...
### Diagram with closed and unlocked as the same thing:
 ...
 ``` mermaid
  ...
  direction LR
  state: "Door"
  open_unlocked --> closed_unlocked : Closing
  closed_unlocked --> open_unlocked : Opening
  closed_unlocked --> closed_locked : Locking
...
```mermaid
   ...
   state: "Door"
   [*] --> Closed and Unlocked
   Closed and Unlocked --> LockUnlock : START
   state LockUnlock {
   [*] --> Unlocked
